...
fun mkRegistry () = HashTable.mkTable (HashString.hashString, (op=)) (313, Event)

fun mkEvent () : event = (ref 0, ref (HashTable.mkTable (Word.fromInt, (op=)) (17, Event)))
    ...
    let fun addListener' (event:string) listener =
		...
		NONE => (HashTable.insert reg (event, mkEvent ());
	      ...
	      | (SOME (n, ref ldic)) => yield (n:= !n + 1;
					       HashTable.insert ldic (!n, listener);
					       !n)
					       
	fun removeListener' event lid =
	    case HashTable.find reg event of
		NONE => yield ()
	      | SOME (n, ref listeners) =>yield (General.ignore (HashTable.remove listeners lid))

	fun raiseEvent' (event, arg) =
	    case HashTable.find reg event of
		NONE => yield ()
	      | SOME (_, ref listeners) => 
		let val tasklist = Vector.fromList (HashTable.listItems listeners)
		in ignore (concurrent (Vector.map (fn t => t arg) tasklist) ()) end
